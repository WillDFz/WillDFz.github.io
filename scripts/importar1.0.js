...
function mediaContinua() {
    ...
    let fi = acmContinua(tratarCsvDiscreta());

    let vetorRaw = tratarCsvDiscreta();
...
function medianaContinua() {
    ...
    let range = rangeClasses(tratarCsvDiscreta());
    let intervalos = range[0];
    let numClasses = range[1];
    let vetTratado = tratarCsvDiscreta();
    let facm = acmContinua(tratarCsvDiscreta());
...
function modaContinua() {
    ...
    let maior = 0;
    let vetorFreqclasses = acmContinua(tratarCsvDiscreta());
...
function desvioPadraoDiscreta() { //só para tabelas quantitativas
    ...
    let vet = tratarCsvDiscreta(); //vetor sem estar tratado, PRECISAMOS DO LENGTH DELE
    let vetorTratado = contador(vet);
    let valores = vetorTratado[0];
    let repeticoes = vetorTratado[1];
...
function desvioPadraoContinua() {
    ...
    let vetorAux = acmContinua(tratarCsvDiscreta());
    let vetorTratado = contador(vet);
    let valores = vetorTratado[0];
    let repeticoes = vetorTratado[1];
...
function tabelaGenerica() {
    if (flag == false) {
        ...
        fiTotal.innerText = tratarCsvDiscreta().length;
        fiPercentTotal.innerText = ((acumularFrequencia(repeticoes) / tratarCsvDiscreta().length) * 100) + "%";
...
function getCSV(file) {
    ...
    let txtCSV = reader.result;

    function a() {
        reader.onload = tratarCsvDiscreta(txtCSV);
    }

}

function tratarCsvDiscreta(txt) {

    // Retirar espacos
    let valor = txt.replace(/\s+/g, '');
    ...
    // Define se e vetor numerico ou de caracteres
    if (isNaN(vetorInt[0]) && isNaN(vetorInt[1])) {
        vetorStr.sort();
        console.log(vetorStr);
        return vetorStr;
    } else {
        selecao(vetorInt);
